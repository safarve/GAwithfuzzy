...
def traintestsplit(X,y):
	i=0;
	X_train,X_test,y_train,y_test=train_test_split(features[i*25:i*25+25], label[i*25:i*25+25], test_size=0.33, random_state=42)
	rge = [i for i in range(657) if i!=0]
	for i in rge:
		X_traintemp, X_testtemp, y_traintemp, y_testtemp = train_test_split(features[i*25:i*25+25], label[i*25:i*25+25], test_size=0.33, random_state=42)
...
#Uncomment only if running for first time, or want to see preprocessing, computationally intensive
path = glob.glob(os.path.join(pathlib.Path(__file__).parent.absolute(),'KannadaHnd\KannadaHnd\Kannada\Hnd\Img\*'))
i=0;
for folder in path:
...
#feature extraction
features=np.empty((16425,168),float)
...
for image in path:
			...
			for l in range(7):
				for k in range(8):
					if(img_box[i,j,l,k]<50):
						...
						angle=angle+math.atan(-k/l)
			sigma[i*4+j]=(1/56.0)*sum
			ang[i*4+j]=(1/56.0)*angle
	mean = np.empty(24,float)
	for i in range(6):
		for j in range(4):
			mean[i*4+j] = img_box[i,j].mean()
	...
	for k in range(6):
		for j in range(4):
			...
			grad[4*k+j]=sum
	features[f_index]=np.concatenate((sigma,ang,mean,diag,sd,xcent,ycent,grad))
	f_index=f_index+1
print("feature extraction complete")
label=np.array([])
for i in range(657):
	label=np.concatenate((label,np.full(25,i)))
...
for i in testlabels:
	...
	if(i!=y_test[j]):
		err= err+1
	j=j+1
print('Accuracy=',100*(err/(len(testlabels))))
